...
class AIPlayer :
    ...
    def saveQTable(self):
        ...
        f.write(str(self.nbPlayedGames)+"\n")
        f.write(str(self.epsilon)+"\n")
        f.write(str(self.gamma)+"\n")
        f.write(str(self.learningSteps)+"\n")
        for i in range(math.factorial(9)) :
            for j in range(4):
    ...
    def makeMove(self, currentState, direction):
        ...
        if direction == self.UP :
            if emptyTileIndex >= 3 :
                ...
                nextState = currentState[0:emptyTileIndex-3] + "9" + currentState[emptyTileIndex-2:emptyTileIndex] + destinationTile + currentState[emptyTileIndex+1:]
        elif direction == self.DOWN :
            if emptyTileIndex <= 5 :
                destinationTile = currentState[emptyTileIndex+3]
                nextState = currentState[0:emptyTileIndex] + destinationTile + currentState[emptyTileIndex+1:emptyTileIndex+3] + "9" + currentState[emptyTileIndex+4:]
        elif direction == self.LEFT :
            if emptyTileIndex%3 != 0 :
                destinationTile = currentState[emptyTileIndex-1]
                nextState = currentState[0:emptyTileIndex-1] + "9" + destinationTile + currentState[emptyTileIndex+1:]
        else :
            if emptyTileIndex%3 != 2 :
                destinationTile = currentState[emptyTileIndex+1]
                nextState = currentState[0:emptyTileIndex] + destinationTile + "9" + currentState[emptyTileIndex+2:]
        if nextState == None :
            nextState = currentState
        return nextState
    
    def getRandomProba(self):
        """
        Returns a random float between 0 and 1.

        :return:    Double corresponding to a probability.
        """
    ...
    def selectNewAction(self, currentState):
        ...
        if self.getRandomProba() < self.epsilon :
            return randint(0,3)
    ...
    def getAlpha(self):
        ...
        """
        return 1/(10**((self.nbPlayedGames+self.learningSteps)/self.learningSteps))
    ...
    def playRound(self, state, action):
        ...
        if nextState == "123456789" :
            ...
            reward = 0
        self.qTable[stateIndex][action] += self.getAlpha() * (reward + self.gamma * max(self.qTable[findRank(nextState)-1]) - self.qTable[stateIndex][action] )
        return nextState
