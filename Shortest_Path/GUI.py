...
class GUI:
    ...
    def __init__(self, history, graph, nodes_pos, is_bidirectional, name, logger):
        self.history = history
        self.is_bidirectional = is_bidirectional
        self.G = graph
        self.nodes_pos = nodes_pos
        self.nodes_pos_without_goals = b = {x: self.nodes_pos[x] for x in self.nodes_pos if x not in [0, 1]}
        self.name = name
        self.logger = logger
        self.index = 0
    ...
    def plotOneState(self, state, iteration, revState=[]):
        ...
        plt.cla()
        ax = plt.gca()
        ax.set_title("{}: iteration {}".format(self.name, iteration))
        ...
        plt.gcf().text(
            0.3, 0.15, "Heuristic + path cost = {:.2f} + {:.2f} = {:.2f}".format(heuristic_cost, path_cost, score), fontsize=11)

        legend_list = [("Current", COLOR_CURRENT),
        ...
        for (text, col) in legend_list:
            ...
            if(final and "Path" in text):
                continue
            plt.gcf().text(x_pos, y_pos, text, color="k", fontsize=10, backgroundcolor=col)
            x_pos += x_step
        ...
        for ix, current_state in enumerate(state[:1] + revState[:1]):
            ...
            for i, k in zip(path[0::1], path[1::1]):
                try:
                    edges_color[self.edges_index.index((i, k))] = COLOR_DST if final else COLOR_PATH
                except ValueError:
                    edges_color[self.edges_index.index((k, i))] = COLOR_DST if final else COLOR_PATH
            nodes_color[current_node] = COLOR_DST if final else current_colors[ix]

        # First draw start and goal nodes with star shape
        nx.draw_networkx_nodes(self.G, self.nodes_pos, nodelist=[
            0, 1], node_color=nodes_color[:2], node_shape="*", node_size=400)
        # Draw all nodes and edges
        nx.draw(self.G, self.nodes_pos, with_labels=True, labels=self.node_labels, font_size=10,
                node_color=nodes_color[2:], nodelist=self.nodes_pos_without_goals, edge_color=edges_color)
        # Add edges labels
        nx.draw_networkx_edge_labels(self.G, self.nodes_pos,
                                     edge_labels=nx.get_edge_attributes(self.G, "weight"), font_size=6)
    ...
    def plotIndex(self):
        if(self.is_bidirectional):
            other_index = self.index+1 if self.index+1 < len(self.history) else self.index-1
            iteration = self.index-1 if self.index > 0 else 0
            self.plotOneState(self.history[self.index], iteration=iteration, revState=self.history[other_index])
        else:
    ...
    def showButtons(self):
        ...
        self.plotIndex()
        self.button_reset = widgets.Button(plt.axes([0.5, 0.01, 0.1, 0.05]), "Reset")
        self.button_back = widgets.Button(plt.axes([0.6, 0.01, 0.1, 0.05]), "Back")
        self.button_next = widgets.Button(plt.axes([0.7, 0.01, 0.1, 0.05]), "Next")
        self.button_end = widgets.Button(plt.axes([0.8, 0.01, 0.1, 0.05]), "End")
