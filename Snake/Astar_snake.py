...
class IA_Astar:
    ...
    def sshape(self, state):
        ...
        if self.first:
            ...
            path.extend((Node((start_x, j)) for j in range(head[1] + 1, len(grid[0]))))
            path.extend(
                (Node((i, len(grid[0]) - 1)) for i in range(start_x - 1, -1, -1))
            )
        ...
        path.extend((Node((i, len(grid[0]) - 1)) for i in range(len(grid) - 1, -1, -1)))
        return path[::-1]
