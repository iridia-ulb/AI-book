...
class GUISnakeGame(SnakeGame):
    ...
    def draw_grid(self, screen, gap, vertical_start, horizontal_start):
        for i in range(self.rows + 1):
            pygame.draw.line(
                ...
                (horizontal_start + self.columns * gap, vertical_start + i * gap),
                1,
            )
    ...
    def draw(self):
        ...
        timer = self.normal_font.render(
            "Timer: " + str(round(self.current_time - self.start_time, 1)), True, WHITE
        )
        self.screen.blit(
            title,
            (
        ...
        if not self.alive:
            self.screen.blit(
                start,
                (
                    menu_start + (width - menu_start) / 2 - start.get_width() / 2,
                    height / 2 - start.get_height() / 2,
                ),
            )

        self.screen.blit(
            timer, (menu_start + (width - menu_start) / 7, height - timer.get_height())
        )
        pygame.display.flip()
