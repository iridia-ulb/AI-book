        ...
        for candidate in range(population_size):
            ...
            for row in range(9):
                for column in range(9):
                    ...
                    initial_board_copy[row][column] = non_existing_numbers_matrix[row][
                        random_index
                    ]
...
class Chromosome:
    ...
    def get_nb_of_duplicates_column(self, column_number):
        ...
        """
        return 9 - len(set([self.board[row][column_number] for row in range(9)]))
    ...
    def selection(self, elite_chromosomes, new_chromosomes_from_crossover):
        ...
        next_population_chromosomes = (
            ...
            + new_chromosomes_from_crossover[
                0 : int((1 - self.elitism_percentage - 0.1) * self.generation_size)
            ]
        )
        new_chromosomes_from_crossover_other_random = new_chromosomes_from_crossover[
            int((1 - self.elitism_percentage - 0.1) * self.generation_size) :
        ]
        ...
        random_population.generate_initial_population(int(0.05 * self.generation_size))
    ...
    def visualSolve(self, wrong):
        ...
        while not finished:
            ...
            for i in range(9):
                ...
                self.board.redraw({}, wrong, time.time() - self.startTime, generation)
