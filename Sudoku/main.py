...
class Board:
    ...
    def __init__(self, window, file=Path("sudokus") / Path("sudoku1.txt")):
        ...
        self.tiles = np.array(
            [
                [
                    Tile(self.board[i][j], window, i * 60, j * 60)
                    for j in range(9)
                ]
                for i in range(9)
            ]
    ...
    def redraw(self, keys, wrong, elapsed, gen=None):
        ...
        for i in range(9):
            for j in range(9):
                if self.tiles[j][
                    i
                ].selected:  # draws the border on selected tiles
    ...
    def hint(self, keys):
        ...
        while (
            True
        ):  # keeps generating i,j coords until it finds a valid random spot
...
def main():
    ...
    if args.file and Path(args.file).is_file():
        ...
        print(
            "No valid file sudoku selected, using default one: sudokus/sudoku1.txt"
        )
        board = Board(screen)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if board.board[selected[1]][selected[0]] == 0 and selected != (
                    -1,
                    -1,
                ):
                    ...
                    if event.key == pygame.K_9:
                        ...
                        if selected in keyDict:
                            ...
                            board.tiles[selected[1]][
                                selected[0]
                            ].value = keyDict[selected]
                            # assigns to actual board so that the correct value can't be modified
                            board.board[selected[1]][selected[0]] = keyDict[
                                selected
                            ]
                            del keyDict[selected]
