...
class Tetris:
    ...
    def reward(self):
        ...
        :return: the reward
        """
        lines_cleared, self.board, deleted_rows = self.check_cleared_rows(self.board)
    ...
    def render(self, video=None):
        ...
        cv2.putText(img, str(self.tetris_score),
        ...
        cv2.putText(img, "Lines:", (self.width * self.block_size + int(self.block_size / 2), 7 * self.block_size),
                    fontFace=cv2.FONT_HERSHEY_DUPLEX, fontScale=1.0, color=self.text_color)
        cv2.putText(img, str(self.total_cleared_lines),
